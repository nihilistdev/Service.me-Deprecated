...
interface TableProps<T> extends React.TableHTMLAttributes<HTMLTableElement> {
  ...
  limit?: number;
  name?: string;
  fn: <T>(...args: any[]) => Promise<T> | T;
}

export const Table = <T,>({ render, fn, name }: TableProps<T>) => {
  const [page, setPage] = React.useState(1);
  const [limit, setLimit] = useLocalStorage("limit", 10);
  const { data, isLoading } = useQuery<PaginatedApiResponse<T> | any>(
    [name || "query", page, limit],
    fn
  );
  const header = Object.keys(data?.results[0] || {}).slice(0, 7);

  const increase = () => {
    if (data?.next && page < data.pages) {
  ...
  return isLoading ? (
    ...
    >
      <svg
        aria-hidden="true"
        className="mr-2 w-1/6 h-1/6 text-gray-200 animate-spin dark:text-gray-600 fill-blue-600"
        viewBox="0 0 100 101"
        fill="none"
    ...
    <div>
      <table className="w-full text-sm text-left text-gray-500 dark:text-gray-400">
        ...
        <tbody>
          {data?.results.map((e: T, idx: number) => (
            <tr
